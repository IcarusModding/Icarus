...
void FDMLPluginDescriptorMetadata::SetupDefaults(const FPluginDescriptor& PluginDescriptor) {
...
void FDMLPluginDescriptorMetadata::Load(const FString& PluginName, const TSharedPtr<FJsonObject> Source) {
    ...
    if (Source->TryGetStringField(TEXT("SemVersion"), SemanticVersion)) {
        ...
        if (bParseSuccess) {
            ...
            UE_LOG(LogDaedalusModLoader, Error, TEXT("Plugin/Mod %s has invalid Semantic Version value: '%s': %s"), *PluginName, *SemanticVersion, *VersionParseError);
        }
    } else {
        UE_LOG(LogDaedalusModLoader, Warning, TEXT("Plugin/Mod %s does not specify 'SemVersion' field, falling back to UE Version"), *PluginName);
    }
    ...
    if (Source->TryGetStringField(TEXT("RemoteVersionRange"), RemoteVersionRangeString)) {
        ...
        if (VersionRange.ParseVersionRange(RemoteVersionRangeString, VersionRangeError)) {
            ...
            UE_LOG(LogDaedalusModLoader, Error, TEXT("Plugin %s has invalid Remote Version Range value: %s: %s"), *PluginName, *RemoteVersionRangeString, *VersionRangeError);
        }
        if (bAcceptsAnyRemoteVersion) {
            UE_LOG(LogDaedalusModLoader, Warning, TEXT("Plugin %s specifies remote version range while also having acceptAnyRemoteVersion set"), *PluginName);
        }
    }
            ...
            if (Item.IsValid() && Item->TryGetObject(ObjectPtr))  {
                ...
                if ((*ObjectPtr)->TryGetStringField(TEXT("SemVersion"), DependencyVersionRangeString)) {
                    ...
                    if (DependencyVersionRange.ParseVersionRange(DependencyVersionRangeString, DependencyErrorMessage)) {
                        ...
                        UE_LOG(LogDaedalusModLoader, Error, TEXT("Plugin %s has invalid dependency '%s' version range '%s': %s"),
...
FDMLPluginDescriptorMetadata UModLoadingLibrary::FindMetadataOrFallback(IPlugin& Plugin) {
    const FDMLPluginDescriptorMetadata* PluginDescriptorMetadata = PluginMetadata.Find(Plugin.GetName());
    ...
    FDMLPluginDescriptorMetadata DefaultMetadata{};
...
void UModLoadingLibrary::VerifyPluginDependencies() {
    ...
    if (MismatchedDependencies.Num()) {
        ...
        UE_LOG(LogDaedalusModLoader, Fatal, TEXT("Found mismatched dependencies versions in the environment. Loading cannot continue: \n%s"), *ErrorList);
    }
#endif
}
...
void UModLoadingLibrary::VerifyPluginDependencies(IPlugin& Plugin, TArray<FString>& MismatchedDependencies) {
    ...
    const FDMLPluginDescriptorMetadata& PluginDescriptorMetadata = PluginMetadata.FindChecked(Plugin.GetName());
    ...
    for (const FPluginReferenceDescriptor& PluginDependency : PluginDescriptor.Plugins) {
        ...
        FDMLPluginDescriptorMetadata DependencyMetadata = FindMetadataOrFallback(*DependencyPlugin.Get());
...
TSharedPtr<FJsonObject> ParsePluginDescriptorFile(IPlugin& Plugin) {
    ...
    if (!FFileHelper::LoadFileToString(FileContents, *PluginDescriptorFilePath)) {
        UE_LOG(LogDaedalusModLoader, Error, TEXT("Failed to open descriptor file %s for plugin %s"), *PluginDescriptorFilePath, *Plugin.GetName());
        return NULL;
    }
    ...
    if (!FJsonSerializer::Deserialize(JsonReader, OutObject)) {
        UE_LOG(LogDaedalusModLoader, Error, TEXT("Failed to parse descriptor file %s for plugin %s, invalid json"), *PluginDescriptorFilePath, *Plugin.GetName());
        return NULL;
    }
...
void UModLoadingLibrary::LoadMetadataForPlugin(IPlugin& Plugin) {
    if (Plugin.IsEnabled() && IsPluginAMod(Plugin) && !PluginMetadata.Contains(Plugin.GetName())) {
        ...
        const FPluginDescriptor& PluginDescriptor = Plugin.GetDescriptor();    
        FDMLPluginDescriptorMetadata PluginDescriptorMetadata{};
...
FVersion UModLoadingLibrary::GetModLoaderVersion() const {
    return FDaedalusModLoader::GetModLoaderVersion();
}

bool UModLoadingLibrary::IsDevelopmentModeEnabled() const {
    return FDaedalusModLoader::GetDMLConfiguration().bDevelopmentMode;
}

TMap<FName, FString> UModLoadingLibrary::GetExtraModLoaderAttributes() const {
    return FDaedalusModLoader::GetExtraAttributes();
}
...
UTexture2D* UModIconStorage::LoadModIcon(const FString& PluginName) {
    ...
    if (LoadedModIcon == NULL) {
        UE_LOG(LogDaedalusModLoader, Error, TEXT("Failed to load icon for plugin %s at file %s: %s"), *PluginName, *Icon128FilePath, *OutErrorMessage);
        return NULL;
    }
