...
TSharedPtr<FJsonObject> FDMLKismetBytecodeDisassembler::SerializeExpression(int32& ScriptIndex) {
	...
	case EX_SetConst:
		{
			...
			FDMLPropertyTypeHelper::ConvertPropertyToPinType(InnerProp, PropertyPinType);

			Result->SetStringField(TEXT("Inst"), TEXT("SetConst"));
			Result->SetObjectField(TEXT("InnerProperty"), FDMLPropertyTypeHelper::SerializeGraphPinType(PropertyPinType, SelfScope.Get()));
	...
	case EX_MapConst:
		{
			...
			FDMLPropertyTypeHelper::ConvertPropertyToPinType(KeyProp, KeyPropPinType);
			Result->SetObjectField(TEXT("KeyProperty"), FDMLPropertyTypeHelper::SerializeGraphPinType(KeyPropPinType, SelfScope.Get()));
				
			FProperty* ValProp = ReadPointer<FProperty>(ScriptIndex);
			FEdGraphPinType ValuePropPinType;
			FDMLPropertyTypeHelper::ConvertPropertyToPinType(ValProp, ValuePropPinType);
			Result->SetObjectField(TEXT("ValueProperty"), FDMLPropertyTypeHelper::SerializeGraphPinType(ValuePropPinType, SelfScope.Get()));
	...
	case EX_LetValueOnPersistentFrame:
		{
			...
			FDMLPropertyTypeHelper::ConvertPropertyToPinType(Property, PropertyType);
			Result->SetObjectField(TEXT("PropertyType"), FDMLPropertyTypeHelper::SerializeGraphPinType(PropertyType, SelfScope.Get()));
	...
	case EX_StructMemberContext:
		{
			...
			FProperty* Property = ReadPointer<FProperty>(ScriptIndex);
			FEdGraphPinType PropertyPinType;
			FDMLPropertyTypeHelper::ConvertPropertyToPinType(Property, PropertyPinType);
			Result->SetObjectField(TEXT("PropertyType"), FDMLPropertyTypeHelper::SerializeGraphPinType(PropertyPinType, SelfScope.Get()));
			Result->SetStringField(TEXT("PropertyName"), Property->GetName());
	...
	case EX_LocalVariable:
		{
			...
			Result->SetObjectField(TEXT("VariableType"), FDMLPropertyTypeHelper::SerializeGraphPinType(PropertyPinType, SelfScope.Get()));
	...
	case EX_Context_FailSilent:
		{
			...
			if (Field) {
				...
				FDMLPropertyTypeHelper::ConvertPropertyToPinType(Field, FieldPinType);
				Result->SetObjectField(TEXT("RValuePropertyType"), FDMLPropertyTypeHelper::SerializeGraphPinType(FieldPinType, SelfScope.Get()));
...
TArray<TSharedPtr<FJsonValue>> FDMLKismetBytecodeDisassembler::SerializeFunction(UStruct* Function) {
...
bool FDMLKismetBytecodeDisassembler::FindFirstStatementOfType(UStruct* Function, int32 StartScriptIndex, uint8 ExpectedStatementOpcode, int32& OutStatementIndex) {
	this->Script = Function->Script;
	this->SelfScope = Function->GetTypedOuter<UClass>();
...
bool FDMLKismetBytecodeDisassembler::GetStatementLength(UStruct* Function, int32 ExpectedStatementIndex, int32& OutStatementLength) {
	this->Script = Function->Script;
	this->SelfScope = Function->GetTypedOuter<UClass>();
...
int32 FDMLKismetBytecodeDisassembler::ReadInt(int32& ScriptIndex) {
...
uint64 FDMLKismetBytecodeDisassembler::ReadQword(int32& ScriptIndex) {
...
uint8 FDMLKismetBytecodeDisassembler::ReadByte(int32& ScriptIndex) {
	uint8 Value = Script[ScriptIndex]; ++ScriptIndex;

	return Value;
}

FString FDMLKismetBytecodeDisassembler::ReadName(int32& ScriptIndex) {
...
uint16 FDMLKismetBytecodeDisassembler::ReadWord(int32& ScriptIndex) {
	uint16 Value = Script[ScriptIndex]; ++ScriptIndex;
	Value = Value | ((uint16)Script[ScriptIndex] << 8); ++ScriptIndex;
	return Value;
}

float FDMLKismetBytecodeDisassembler::ReadFloat(int32& ScriptIndex) {
	union { float f; int32 i; } Result;
	Result.i = ReadInt(ScriptIndex);
	return Result.f;
}

CodeSkipSizeType FDMLKismetBytecodeDisassembler::ReadSkipCount(int32& ScriptIndex) {
...
FString FDMLKismetBytecodeDisassembler::ReadString8(int32& ScriptIndex) {
	FString Result;
...
FString FDMLKismetBytecodeDisassembler::ReadString16(int32& ScriptIndex) {
	FString Result;
...
FString FDMLKismetBytecodeDisassembler::ReadString(int32& ScriptIndex) {
