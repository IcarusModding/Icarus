...
FString UBlueprintAssetHelperLibrary::FindPluginNameByObjectPath(const FString& ObjectPath, bool bTreatNonModPluginsAsGame) {
	...
	if (PackageMountPoint == TEXT("None")) {
		UE_LOG(LogDaedalusModLoader, Error, TEXT("FindPluginNameByObjectPath: received invalid path with no associated mount point: %s"), *ObjectPath);
		return TEXT("");
	}

	//Game mount point is owned by Icarus itself, since we now have a strong content separation between mods and the game
	...
	//and we can figure out module owner by iterating plugin manifests and assuming Icarus if it's not found
	if (PackageMountPoint == TEXT("Script")) {
		...
		if (!FModuleManager::Get().IsModuleLoaded(*PackageName)) {
			UE_LOG(LogDaedalusModLoader, Error, TEXT("FindPluginNameByObjectPath: Found /Script/ package not representing a loaded module: %s"), *PackageName);
			return TEXT("");
		}
		return FindOwnerPluginForModuleName(PackageName, bTreatNonModPluginsAsGame);
	}

	//Assume all configs to be owned by the Icarus, and for Memory/Temp we cannot really make a distinction,
	//so we assume they are Engine-owned, and belong to Icarus too
	...
	if (ResultName.IsEmpty()) {
		UE_LOG(LogDaedalusModLoader, Error, TEXT("FindPluginNameByObjectPath: Encountered unexpected mount point: %s"), *PackageMountPoint);
		return TEXT("");
	}
