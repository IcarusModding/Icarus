 ...
 * declared in the plugin file for DML
 */
USTRUCT(BlueprintType)
struct DML_API FModInfo {
    GENERATED_BODY()
    /**
     ...
     * Replaces Mod Reference from DML 2.x.x versions and follows the same rules
     */
     ...
     * If the mod does not declare DML metadata, it's
...
/** Contains plugin descriptor metadata read and used by DML to provide extra functionality */
struct DML_API FDMLPluginDescriptorMetadata {
...
UCLASS()
class DML_API UModLoadingLibrary : public UEngineSubsystem {
    GENERATED_BODY()
public:
    UModLoadingLibrary();
    
    /** Returns true when mod with the provided codename is loaded */
    UFUNCTION(BlueprintPure, Category = "DML|Mod Loading", meta = (BlueprintThreadSafe))
    ...
    UFUNCTION(BlueprintCallable, Category = "DML|Mod Loading")
    UTexture2D* LoadModIconTexture(const FString& Name, UTexture2D* FallbackIcon);

    /** Returns the currently used DML version */
    UFUNCTION(BlueprintPure, Category = "DML|Mod Loading", meta = (BlueprintThreadSafe))
    ...
    /** Reloads DML-related plugin metadata for active plugins */
...
private:
    ...
    FDMLPluginDescriptorMetadata FindMetadataOrFallback(IPlugin& Plugin);
    ...
    /** Called when new plugin is created, used to preload DML metadata for it */
    ...
    TMap<FString, FDMLPluginDescriptorMetadata> PluginMetadata;
};

/** Holds mod icons and manages their loading */
UCLASS(Transient)
class DML_API UModIconStorage : public UObject {
    GENERATED_BODY()
private:
